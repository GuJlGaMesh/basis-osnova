...
function functions(extra = {}) {
    ...
    const context = { ...extra, Vector: (x, y, z) => ({ x, y, z }) };
...
for (const centerZ of [100, 105, 105.01, 200]) for (const sign of [1, -1]) {
    test(`Контур Z=${centerZ}, знак локальной оси=${sign}: окружность, стыки и площадь`, () => {
        ...
        for (const s of c.specs.filter(s => s.cut === 'round-open')) {
            ...
            assert.equal(edges.length, centerZ <= 105 ? 6 : 8, 'Ожидался круговой сегмент либо U-образный проход');
            let area = 0;
...
function preflight(answers, options = {}) {
    ...
    const alerts = [], calls = { delete: 0, build: 0, open: 0, paths: [] };
    const forbiddenBuild = () => { calls.build++; throw new Error('CAD построение в тесте запрещено'); };
    const context = {
        Model: fixture.Model, DatumMode: { Box: 6, FaceFace: 2 },
        AddBlock: forbiddenBuild, AddCopy: forbiddenBuild, AddExtrusion: forbiddenBuild,
        ...
        OpenFurniture: file => {
            calls.open++; calls.paths.push(file);
...
test('ASCII без BOM: все байты выдаваемого скрипта меньше 128', () => {
    assert.ok(fs.readFileSync(sourcePath).every(byte => byte < 128));
});
test('Без глобального TFastener ввод и сообщения работают', () => {
    const result = preflight([null]);
    assert.match(result.message, /Ввод отменён/);
    assert.equal(result.calls.delete, 0);
    assert.ok(!result.message.includes('Рџ'));
});
test('Недоступный API назван явно', () => {
    const result = preflight(['1', '100', '120'], { noAPI: true });
    assert.match(result.message, /API: AddVertPanel\./);
    assert.equal(result.calls.delete, 0);
});

test('Без Geometry2D.Area доступны ввод и проверка библиотек', () => {
    const result = preflight(['1', '100', '120']);
    assert.equal(result.calls.open, 1);
    assert.equal(result.calls.delete, 0);
    assert.match(result.message, /проверка библиотеки Firmax/);
    assert.ok(!result.message.includes('Geometry2D'));
});

test('Firmax300 загружается по точному пути установленной размерной библиотеки', () => {
    const expected = JSON.parse(fs.readFileSync(path.join(__dirname, 'firmax-300-installed-source.json'), 'utf8'));
    const result = preflight(['1', '100', '120']);
    assert.equal(result.calls.paths[0], expected.source_path);
    assert.ok(!/[\x00-\x1f]/.test(result.calls.paths[0]));
    assert.match(result.message, /L=300 мм/);
});
...
const summary = {
    ...
    limits: [
        ...
        'Geometry2D отсутствует во всех тестовых контекстах. Дуги проверены независимым интегралом через три точки; скрипт считает площадь передаваемых координат через центр и sweep. Трактовка команд и булева операция круга самим БАЗИС не проверены.',
