...
try {
 Action.SaveModel(base+'before-append-r04.b3d');
 writeReport(base+'r04-before.json',inspectModel(Model,'scene'));
 var first=buildModel4({dimensions:[450,712,513]});
 writeReport(base+'r04-once.json',inspectModel(Model,'scene'));
 var second=buildModel4({dimensions:[800,862,600]});
 writeReport(base+'r04-twice.json',inspectModel(Model,'scene'));
 Action.SaveModel(base+'model4-r04-scene.b3d');
 Action.LoadModel(base+'model4-r04-scene.b3d');
 writeReport(base+'r04-reopened.json',inspectModel(Model,'scene'));
 Model.UnSelectAll();Model.Objects[Model.Count-1].Objects[1].Objects[7].Selected=true;
 writeReport(base+'r04-complete.json',{done:true});
}catch(e){writeReport(base+'r04-error.json',{error:String(e)});}
