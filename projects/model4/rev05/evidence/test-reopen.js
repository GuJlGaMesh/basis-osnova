...
try {
 Model.UnSelectAll();
 Action.SaveModel(base+'model4-r05-scene.b3d');Action.LoadModel(base+'model4-r05-scene.b3d');
 writeReport(base+'r05-scene-reopened.json',inspectModel(Model,'scene'));
 var selections=[];function walk(list){for(var i=0;i<list.Count;i++){var o=list.Objects[i];if(o.Contour!==undefined&&o.Butts!==undefined){Model.UnSelectAll();o.Selected=true;selections.push({name:o.Name,fasteners:o.FindConnectedFasteners().length});}if(o.List)walk(o);}}
 walk(Model);Model.UnSelectAll();writeReport(base+'r05-selection-check.json',{panels:selections});
 ViewAll();
}catch(e){writeReport(base+'r05-reload-error.json',{error:String(e)});}
