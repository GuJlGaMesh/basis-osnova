...
7. Генерируй скрипты в режиме полной замены содержимого текущей модели: проверка входов → очистка → построение заново. Не требуй пустую модель и не блокируй запуск сообщением «откройте пустую модель»; подробности — `docs/development/bazis.md`.
