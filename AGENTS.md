...
5. Результат задачи моделирования — готовый `.js` и комплект по `docs/development/releases.md`. Человек вручную открывает и запускает его в БАЗИС на другом компьютере; отсутствие локального БАЗИС или управления его окном не блокирует подготовку скрипта.
